...
class Database
{
    ...
    */
    ...
    public function insert($table, $data, $commit=OCI_COMMIT_ON_SUCCESS) {
        ...
        $fildNames = implode(',', array_keys($data));
        $fildValues = ':' . implode(', :', array_keys($data));

        $result = oci_parse($this->link, "INSERT INTO $table ($fildNames) VALUES ($fildValues)");
    ...
    public function update($table, $data, $where, $commit=OCI_COMMIT_ON_SUCCESS)
    {
        ...
        foreach($data as $key => $value) {
            $teste[$key] = $value; //bug na passagem por referencia
            oci_bind_array_by_name($result, ":$key", $teste[$key], -1);
        }
